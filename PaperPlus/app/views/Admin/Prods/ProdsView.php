<link rel="stylesheet" href="app/CSS/MainStyles.css">
<script src="app/JS/ProdsFunc.js"></script>
<div class="section">
    <div class="content-RegistroUser">
        <h4>Añadir Nuevo Producto</h4>
        <div class="Usuario-img">
            <img src="app/img/prods.png" alt="#">
        </div>
        <form action="/PaperPlus/?C=Prods_Controller&M=RegNew" method="post" enctype="multipart/form-data">
            ...
            <label for="ProdCat">Selecciona Categoria: </label>
            <select name="ProdCat" id="Cat" class="control">
                <?php foreach($consultaCat as $rowsc){ ?>
                    <option value="<?php echo $rowsc->IdCategoria; ?>"><?php echo $rowsc->Nombre; ?></option>
                <?php } ?>
            </select>
            <label for="ProdsMark">Selecciona la Marca: </label>
            <select name="ProdMark" id="Mark" class="control">
        ...
        <table class="centered-table">
            <thead>
                ...
                foreach ($consulta as $prods) {
                    echo '<form class="form" action="/PaperPlus/?C=Prods_Controller&M=EditxDel" method="POST" enctype="multipart/form-data">';
                    echo '<tr>';
                    echo '<td> <input type="text" name="IdProd" value="'.$prods->IdProd.'" readonly></td>';
                    ...
                    echo '<td> <button type="submit" name="bttnEdit" value="BttnAct">Editar</button><br><button type="sybmit" name="bttnDel" value="Delete">Eliminar</button></td>';
                    echo '</tr>';
                    echo '</form>';
                }
                ?>
